...
uint8_t num_mas[] = { // массив цифр для индикатора
		0x3F, // 0
		0x06, // 1
		0x5B, // 2
		0x4F, // 3
...
void TM1638_init(uint8_t brightness)
{
	...
	// используется младшая половина байта. Первые  три отведены на яркость и четвертый на управление включением
...
void TM1638_Led_OnOff( uint8_t led, LedStatus status ){
	...
	// отправляем данные вторым байтом ( чтоб зажечь светодиод нужен 0x01 ) первый бит препаян к светодиоду
	HAL_SPI_Transmit( &TM1638_SPI_HAL, &buf, 1, HAL_MAX_DELAY );
...
uint8_t TM1638_readKey(void){
	...
	поетому не все биты нужны, есть модели где распаяны все кнопки 
	...
	*/
	...
	// после того как считали значение нужно обратно записать любое цисло ( нужно для тактирования )
	cmd = 0xFF;
	HAL_SPI_Transmit( &TM1638_SPI_HAL, &cmd , 1, HAL_MAX_DELAY );
	
	TM1638_Unselect();
	
	cmd = 0; // обнуляем переменную и в нее будем формировать биты нажатых кнопок
